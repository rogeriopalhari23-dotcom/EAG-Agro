...
**Versão:** 2.0 — rascunho 4 da Fase 3
**Data:** 2026-09-22
**Status:** em revisão; **não aprovada integralmente**. Decisões de Rogério em 2026-09-22 incorporadas: perfil Administrador, B2, B3 em parte, DS1 (DS1-b não, DS1-c sim), DS2 e DS3; nova leitura T12 (rev. 3) e diretriz "todo trabalho de prospecção obedece à `/prospeccao-vendas`". Fase 3 não encerrada.
**Rascunhos anteriores:** `docs/historico/v2.0-planejamento/eag-compass-spec-v2.0-rascunho1.md` e `…-rascunho2.md` e `…-rascunho3.md`.
**Versão substituída:** Especificação v1.3 (2026-09-21), arquivada sem alteração em `docs/historico/v1.3/eag-compass-spec-v1.3.md`.
**Constituição associada:** `docs/eag-compass-constituicao.md` (v2.0, rascunho 4).
...
| **T12** | `docs/eag-compass-t12-prospeccao-vendas.md` | revisão 3 (leitura da skill `33bd093f…9dd8`) |
...
| T12 | `/prospeccao-vendas` | Versão 1.0.0 do curso EAG Agro (SHA-256 `33bd093f…9dd8`) **lida nesta sessão** e registrada no T12 rev. 3; PV1–PV12 e R28 especificados. Conflitos K1–K6 pendentes. Gerador, revisor e amostras não implementados. A v1.0 anterior (`94544246…acd0`) foi substituída. | Antes da primeira ficha do piloto |
...
- **R14.3 (revisão pendente de K1/K3):** A priorização DEVE ordenar consumidores finais antes de traders/distribuidores, sem excluí-los da visualização. **Até a decisão de K1/K3:** ficha permitida só para indústrias usuárias de porte médio ou média-mais (ICP da skill, I3); pequenas/MEI, gigantes sem relacionamento prévio e traders ficam visíveis e classificados, sem ficha (T12 §3.2).
...
- **R17.8:** SE o SHA-256 da skill disponível for diferente do incorporado, ENTÃO o sistema DEVE impedir a geração de novos textos até nova leitura registrada (T12) e revisão de PV/A. Textos já aprovados não são reescritos (R17.5). *Situação em 2026-09-22: nova leitura registrada (T12 rev. 3). A referência passa a `33bd093f…9dd8` quando esta Spec for aprovada; até lá, nenhuma geração.*
...
#### R28 — ICP, lista, cadência, ligações e reunião conforme a skill [Interno; tarefas manuais dependem de K4; abertura depende de K5, T1 e T11]
...
- **R28.5 [K4]:** A campanha DEVE usar o modelo de cadência da skill (2 semanas, lote de 10–15 empresas). E-mails aprovados são enviados pelo sistema; ligações e LinkedIn viram tarefas manuais para Rogério no dia previsto, com o roteiro correspondente. Até a decisão de K4, só os e-mails.
- **R28.6 [K4]:** SE não houver e-mail validado do decisor, ENTÃO o sistema DEVE criar tarefa de cold call Level 0 (descobrir nome e e-mail do comprador) em vez de iniciar a sequência. Até K4, a empresa fica com a pendência "sem destinatário validado".
- **R28.7 [K5]:** QUANDO o provedor sinalizar abertura de um e-mail da sequência, o sistema DEVE registrar "abertura sinalizada" (nunca "lido") e criar tarefa "ligar agora" para Rogério, sem enviar nada automaticamente. Até K5, T1 e T11, não há rastreamento de abertura.
...
| AT59 | R14.3 | DADO empresa MEI, e outra classificada como trader, ENTÃO ambas aparecem no Radar com a classificação, e a ficha é recusada enquanto K1/K3 estiverem pendentes. |
...
| **K1** | Porte do ICP: médias/média-mais (skill I3) × PMEs (decisão 3) | Ficha só para médias/média-mais; pequenas/MEI e gigantes sem relacionamento: visíveis, sem ficha (R14.3) | Fichas fora desse porte |
| **K2** | 1–2 commodities ativas (I3) × "uma ou mais" no Internacional (decisão 2) | Máximo de 2 commodities com campanhas ativas por mercado | Terceira campanha ativa |
| **K3** | Traders fora do ICP da skill × traders elegíveis (decisão 3) | Traders visíveis e classificados, sem ficha (R14.3) | Fichas para traders |
| **K4** | Cadência multicanal da skill × piloto "somente e-mail" (ESC Etapa 1) | Só e-mails; sem tarefas manuais de ligação/LinkedIn (R28.5, R28.6) | Cadência completa da skill |
| **K5** | "Abriu → ligar na hora" (I5 ⚡) × rastreamento de abertura (LGPD/T11, T1, R19.5) | Sem rastreamento; I5 inativa com aviso (R28.7) | Regra I5 |
| **K6** | Frases de volume disponível e prova social nos roteiros × nada comercial sem dado aprovado | Frases omitidas, estrutura mantida (PV1, PV7) | E-mail 1 completo conforme roteiro |
