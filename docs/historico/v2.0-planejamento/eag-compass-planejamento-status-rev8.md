...
**Iniciado:** 2026-09-22
**Atualização:** 2026-09-22 — revisão documental 8 do status (Fase 3, rascunho 4: nova leitura T12 e diretriz de prospecção)
**Revisões anteriores:** rev. 4 a 7 arquivadas em `docs/historico/v2.0-planejamento/`.
**Método:** `/planejar`
...
| **T12** | **Rev. 3: skill atual (`33bd093f…9dd8`) lida nesta sessão; PV1–PV12 e R28 especificados; K1–K6 aguardam decisão.** Implementação e amostras pendentes. | `docs/eag-compass-t12-prospeccao-vendas.md` |
| **3. Spec + Constituição v2.0** | **Rascunho 4. Não aprovadas integralmente; fase aberta.** | `docs/eag-compass-spec.md`, `docs/eag-compass-constituicao.md` |
...
Estas mensagens **não** aprovaram integralmente a Spec e a Constituição nem encerraram a Fase 3.
...
Documentos vigentes após esta revisão: Spec rascunho 4 `fc7c59a866befa19c401043b5f25cc7b8edf7748ed14045d68188a4601e0e935`; Constituição rascunho 4 `0795a490b4c4cbc19589c44f5b315f2f9d2827c37286e1b87a447a9df7d61d23`; T12 rev. 3 `120157c2293a4d14081103e118f160327c73959934b8ed8d3d2b1c825b91f1ec`.
...
- AT1–AT60 contínuos (AT51–AT60 novos no rascunho 4).
...
- Verificação mecânica: 305 referências na Spec, 67 na Constituição e 20 no T12, todas resolvidas; nenhum ID duplicado; K1–K6 presentes na Spec e no T12.

## Revisão de consistência — rascunho 4

| # | Achado | Situação |
| --- | --- | --- |
| C1 | Escopo rev. 3 diz que a skill não estava disponível e usa "PMEs" e "piloto somente por e-mail". | Escopo congelado. A defasagem sobre a skill está resolvida pelo T12 rev. 3; porte e canais do piloto viraram K1 e K4. |
...
| C9 | Cinco decisões preservadas × Spec. | Sem contradição direta. **Decisão 3 (traders elegíveis) e decisão 2 ("uma ou mais commodities") agora colidem com o inviolável I3 da skill → K3 e K2.** |
...
| C16 | DS1-c × porte-alvo. | Com K1, o porte-alvo pode mudar para médias/média-mais; `M_N` deve ser calibrado para esse porte. |
| C18 | Roteiros da skill incluem "volume disponível", "já atendemos empresas de grande porte", "reduz custo de aquisição", "preço competitivo". | K6. Até a decisão, as frases ficam fora; a estrutura é mantida (PV1, PV7). |
| C19 | I5 "abriu → ligar na hora" exige rastreamento de abertura. | K5 (T1 + T11). Até lá, regra inativa com aviso. |
...
| --- | --- | --- | --- |
| **K1** | Porte do ICP: médias/média-mais (skill) × PMEs (escopo) | Ficha só para médias/média-mais | Fichas de outros portes |
| **K2** | Até 2 commodities ativas por mercado × "uma ou mais" no Internacional | Máximo 2 ativas | 3ª campanha ativa |
| **K3** | Traders fora do ICP da skill × traders elegíveis (decisão 3) | Visíveis, sem ficha | Fichas de traders |
| **K4** | Cadência multicanal da skill × piloto só e-mail | Só e-mails | Tarefas de ligação/LinkedIn |
| **K5** | "Abriu → ligar na hora" × rastreamento de abertura | Sem rastreamento | Regra I5 |
| **K6** | Frases de volume disponível e prova social nos roteiros | Omitidas | E-mail 1 completo |
...
| Comerciais | Commodity, cidade/UF e raio do piloto; CSO; valores `M_N`; declarações aprovadas (se K6 aceitar) | — | Etapa 1 |
| T11 | Retenção/exclusão; pixel de abertura, se K5 aceitar | — | Contatos reais |
| Git | Destino da troca da base 0.3.1 | — | Fase 6 |

## Próximo passo

1. Rogério decide K1–K6 (B1 pode continuar pendente) e aprova ou ajusta o rascunho 4.
2. Com aprovação expressa: marcar a Fase 3 como concluída.
3. Fase 4: stack e conta Cloudflare (T9/T10), provedor de e-mail com validação de endereço e, se K5, sinal de abertura (T1), fontes da lista pelo uso final (T4/T8), limites de raio (B3), pipeline de publicação.
