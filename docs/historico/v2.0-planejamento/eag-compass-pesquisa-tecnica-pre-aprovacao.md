...
**Status:** em andamento. **Aprovadas por Rogério em 2026-09-22: G1, G3 (Hostinger, risco assumido), G4 (IMAP), G5, G8–G11, volume.** G2 e G7 encerradas. Pendente: G6 (recursos antigos) e o fechamento da fase (cobertura de skills + aprovação da stack). Limites de raio (B3) decididos. G2 encerrada (Compass na conta de Rogério, sem a zona `eagagro.com`). **G3 e G4 reabertas.** Pendentes: G3, G4, G6–G11.
...
**Aprovada por Rogério em 2026-09-22 — proposta para o piloto** (domínio principal, sem domínio dedicado — decisão de Rogério):
...
| --- | --- | --- |
| **G3 — decidida (2026-09-22)** | **Envio pela caixa Hostinger, sem consulta prévia; risco do §12 assumido por Rogério.** Mitigações: volume §7.8 (R19.10–R19.12), descadastro (R21.7), remetente/reply-to legítimos e endereço físico da EAG (R19.13) | — |
| **G4 — decidida** | Recebimento por **IMAP** (`imap.hostinger.com:993`), correlação por `In-Reply-To`/`References` | — |
| **G7 — encerrada** | DMARC `p=none` mantido: sem acesso ao DNS do `eagagro.com` (administrador desconhecido) | — |
| **G8–G11 — aprovadas (2026-09-22)** | Casa dos Dados (lista), centroide + LocationIQ pago (coordenadas), PMTiles no R2 + MapLibre + turf (mapa), Snov.io (validação; catch-all/unknown não valida) | — |
| G3 — atualização (histórico) | **Rogério descartou domínio dedicado** (G3-c fora). Como o DNS do `eagagro.com` está numa conta Cloudflare de administrador desconhecido (G2), também não é possível autorizar outro provedor (SPF/DKIM) a enviar pelo domínio. **Resta a caixa Hostinger** (E1) como único transporte viável, com a questão contratual do §12 em aberto | Decidir entre: (a) consultar a Hostinger por escrito antes do 1º envio real; (b) seguir assumindo o risco. Em qualquer caso, cumprir o que o §12 lista: descadastro em toda mensagem (já R21.7), endereço físico do remetente na assinatura, remetente e reply-to legítimos, envio 1 a 1 e em baixo volume |
...
## 10. Cobertura de skills da stack (passo 7 da Fase 4)

| Componente | Skill disponível | Situação |
| --- | --- | --- |
| Workers (API, Cron, TCP `connect()`), `wrangler.jsonc` | `cloudflare-atlas`, `cloudflare:workers-best-practices`, `cloudflare:wrangler` | Dedicada |
| D1, KV, Queues, R2 | `cloudflare-atlas` (referências por produto), `cloudflare:cloudflare` | Coberta |
| Access + `ctx.access` | `cloudflare:cloudflare-one` | Dedicada |
| Segurança do código | `security-review` | Coberta |
| Testes (`node --test`), TDD | `superpowers:test-driven-development` | Coberta |
| Método de prospecção (textos, cadência, lista) | `/prospeccao-vendas` (obrigatória, T12) | Dedicada |
| Pesquisa de fornecedores e fontes | `/search`, `/anthropic-skills:pesquisa`, MCP `mcp-brasil` | Coberta |
| MapLibre GL JS + PMTiles + turf | Candidatas no catálogo `skills.sh` com **1 instalação** cada (`mapsnippets/maplibre-skill`, `maplibre/maplibre-compose`) | **Gap** — não instalar (baixa confiabilidade); auditar com `context7` e docs oficiais Protomaps |
| Cliente SMTP/IMAP num Worker (Hostinger) | Candidatas com **1 instalação** (`smithery.ai/imap-email`, `skills.volces.com/imap-idle`) | **Gap** — não instalar; auditar com RFCs e `context7` da biblioteca escolhida |
| APIs externas (Casa dos Dados, LocationIQ, Snov.io) | Sem skill | **Gap** — contrato de cada tarefa cita a doc oficial (Fase 6, item 2) |

**Scaffold (`cloudflare/templates`):** não aplicável como ponto de partida. O produto já tem base própria (0.3.1: Worker JS, D1, assets); o plano descreve o delta sobre ela (rota "refactoring de produto existente" da `/planejar`).

## 11. Stack proposta para aprovação (fechamento da Fase 4)

| Camada | Decisão | Plano / custo estimado |
| --- | --- | --- |
| Hospedagem, API, cron | Cloudflare Workers + Static Assets (conta de Rogério, `workers.dev`) | Workers Paid US$ 5/mês (G1) |
| Dados | D1 (reaproveitar `eag_compass` vazio), KV, Queues, R2 (habilitar) | Incluído / free tier |
| Autenticação | Cloudflare Access + `ctx.access.getIdentity()`; perfis no D1 | Free (limite de usuários a validar) |
| Envio | SMTP da caixa Hostinger (465), exceção P19, risco §12 assumido; volume 5→20/dia | Plano Hostinger atual |
| Respostas | IMAP da caixa Hostinger (993) por Cron | — |
| Lista | Casa dos Dados API | R$ 29,90–59,90/mês (a validar a contagem de "consultas") |
| Coordenadas | Centroide do município + LocationIQ pago para candidatos | ~US$ 45/mês (◐) |
| Mapa | PMTiles no R2 + MapLibre + turf | Free tier R2 |
| Validação de e-mail | Snov.io | A validar |
| Pendente | G6 — destino do Worker `eag-compass-production` e das filas antigas (decidir no plano, sem exclusão agora) | — |
