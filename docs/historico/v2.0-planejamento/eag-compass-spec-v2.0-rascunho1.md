...
**Versão:** 2.0 — rascunho 1 da Fase 3
**Data:** 2026-09-22
**Status:** redigida para revisão; **não aprovada**. Nenhuma aprovação formal foi registrada em nome do usuário.
**Versão substituída:** Especificação v1.3 (2026-09-21), arquivada sem alteração em `docs/historico/v1.3/eag-compass-spec-v1.3.md`.
**Constituição associada:** `docs/eag-compass-constituicao.md` (v2.0, rascunho 1).
...
| **T12** | `docs/eag-compass-t12-prospeccao-vendas.md` | revisão 2 |
...
| **[Proposta Bn]** | Redação pendente de decisão de Rogério (BEN §7). Não é regra aprovada; a implementação não começa por ela. |
...
| T12 | `/prospeccao-vendas` | 1º marco relatado (leitura v1.0, SHA-256 registrado); regras especificadas nesta Spec (R17); implementação e amostras pendentes | Antes da primeira ficha do piloto |
...
### 1.2 Estados — **Revisado**

A v1.3 tinha um único pipeline iniciando em `Descoberto` e exigindo evidência empresarial para `Prospectado`. A v2.0 separa três trilhas (ESC §2.3: primeiro contato ≠ qualificação ≠ proposta específica).

**Trilha de prospecção** — por **empresa + commodity**:

| Estado | Entrada |
| --- | --- |
| Descoberto | Empresa registrada por busca (R11/R12) ou manualmente (R1.1), com fonte |
| Em triagem | Usuário abriu a revisão da empresa para a commodity |
| Ficha em aprovação | Ficha versionada completa aguardando decisão (R18) |
| Ficha aprovada | Ficha aprovada; sequência pode ser ativada se as verificações pré-envio passarem (R19) |
| Em abordagem | Pelo menos um passo da sequência aceito pelo provedor |
| Em atendimento | Resposta humana recebida; contatos automáticos suspensos (R20) |
| Sequência concluída | Último passo aprovado executado sem interrupção |
| Adiado / Descartado | Decisão humana com motivo (R18.2) |

**Trilha de qualificação** — por **empresa + demanda**, inicia apenas por decisão humana após resposta ou contato registrado. Mantém os estados v1.3 **Em Contato → Em Qualificação → Qualificado → Oportunidade Confirmada**, com as entradas da v1.3 e o gate revisado em R7.2.

**Estado `Prospectado` (v1.3): Substituído.** Na v1.3 significava "evidência empresarial válida". Na v2.0 a evidência empresarial é pré-requisito do gate de qualificação (R7.2 item 1), não da abordagem inicial. Registros migrados de `Prospectado` mantêm a evidência e passam a `Descoberto` na trilha de prospecção.

**Trilha de oferta específica** — opcional, por oferta versionada: Rascunho → Vigente → Alterada/Vencida → Encerrada (R16.3).
...
- **R1.3.2:** Após validação, o sistema DEVE recalcular Confidence (R5) e preservar evidências anteriores, inclusive conflitantes.
- **Removido:** a transição automática para `Prospectado` (ver §1.2).
...
- **R2.4.2:** O sistema NÃO DEVE bloquear abordagem, ficha ou qualificação apenas porque o perfil é trader/distribuidor.
...
- **R3.1.4:** Preço indicativo continua opcional e DEVE registrar moeda, unidade, validade e fonte. **USD é a moeda-base no Internacional; BRL é a moeda-base no Nacional** (decisão a confirmar — ver DS2 no §6).
...
- **R3.4.2:** As perguntas DEVEM variar conforme o perfil comprador (consumidor final: uso; trader: compra/revenda; desconhecido: confirmação de perfil) — T12 A10.
- **R3.4.3:** Sugestões não são respostas e não alteram scores sem confirmação (mantido). O framework "Estrutura, Análise, Gestão" da skill NÃO DEVE redefinir os campos do formulário EAG (T12 A9).
...
Tabelas da v1.3 mantidas como regra do mercado Internacional. Uso no Nacional depende de DS1.
...
- **R5.1.2:** A dimensão "Evidência de compra" no mercado Nacional segue DS1 (§6). Até a decisão, o sistema DEVE aplicar a tabela v1.3 sem conceder pontos por registro alfandegário/BL a compras domésticas e DEVE exibir a nota "regra nacional de Confidence pendente".
...
| --- | --- | --- |
| `param_volume_min[commodity, mercado, oferta?]` | sem valor salvo onde não configurado | Substitui `[commodity, supplier?]` |
| `param_radius_km_min`, `param_radius_km_max` | **sem valor aprovado** | Proposta B3 |
| `param_period_default_months` (Internacional) | **a definir em T6** | Pendente |
| `param_send_daily_limit[canal, remetente]` | **a definir em T1** | Pendente |
...
- **R11.9:** A ordenação padrão DEVE priorizar aderência e PMEs consumidoras finais; traders e distribuidores DEVEM continuar visíveis e classificados, com prioridade secundária, respeitando filtros e condições confirmadas.
...
- **R11.11:** SE o raio informado estiver fora de `param_radius_km_min`/`max`, ENTÃO o sistema DEVE recusar a busca e exibir o intervalo permitido. **[Proposta B3 — sem valores aprovados; enquanto não houver, R7.1.1 bloqueia a busca.]**
- **R11.12:** QUANDO o usuário alterar o raio de uma busca concluída, o sistema DEVE criar nova busca vinculada à mesma origem, preservando a anterior. [Proposta B3]
...
- **R14.3:** A priorização DEVE ordenar consumidores finais PME antes de traders/distribuidores, sem excluí-los.
...
- **R15.1:** O sistema DEVE registrar para cada decisor: nome, cargo, papel mapeado (econômico, técnico, usuário, coach, desconhecido), fonte, data e grau de confirmação; cargo não implica autoridade (R2.3).
...
- **R17.1:** Cada texto gerado DEVE registrar o SHA-256 do `SKILL.md` usado (`94544246…acd0` na leitura v1.0), a versão das adaptações Compass (A1–A11) e a versão do gerador.
...
- **R17.3:** O revisor automático DEVE verificar os critérios PV1–PV8 abaixo e exibir cada violação na ficha. SE houver violação não resolvida, ENTÃO o sistema DEVE impedir a aprovação da ficha.

| ID | Critério verificável | Origem |
| --- | --- | --- |
| PV1 | Primeiro e-mail contém uma pergunta curta de Situação/Problema **ou** pedido de encaminhamento ao responsável, adequada ao perfil (uso / compra-revenda / confirmação de perfil) | Módulo 3 + A10 |
| PV2 | Primeiro e-mail não abre com Implicação nem com lista longa do portfólio | Módulo 3 |
| PV3 | Cada mensagem tem exatamente um objetivo declarado na ficha | Checklist |
| PV4 | Cada afirmação personalizada aponta para evidência registrada (fonte + data); menção a contato anterior exige evento realizado | A2, A3 |
| PV5 | Cada follow-up tem objetivo próprio, pergunta adequada ao perfil, sem inferir dor, volume ou interesse | Módulos 4 e 6 |
| PV6 | Mensagem de encerramento só é enviada no passo final sem resposta, supressão, bloqueio ou pausa | Módulos 5 e 6 + A11 |
| PV7 | Nenhum texto contém preço, lote, estoque, prazo, certificação, pagamento, concorrente ou fornecedor atual do comprador sem dado aprovado | A4, A6 |
| PV8 | Destinatário e argumento coerentes com o papel mapeado, sem afirmar autoridade não confirmada | Módulos 2 e 8 |

- **R17.4:** Textos DEVEM usar apenas a estrutura dos frameworks; conteúdo vem do catálogo, das evidências e da campanha (A7). Módulo 7 (negociação) NÃO DEVE ser usado em texto automático (A6).
...
- **R17.6:** Antes da primeira ficha real do piloto, amostras internas DEVEM cobrir consumidor final, trader, perfil desconhecido e sequência interrompida antes do encerramento, com revisão registrada por Rogério.
...
- **R20.7 [Proposta B1]:** SE a mensagem for ausência automática identificada, ENTÃO o sistema DEVE pausar apenas a pessoa, sem contar como resposta comercial. Retomada automática só DEVE ocorrer se prevista na ficha, com data de retorno e fuso inequívocos e todas as verificações de R19.2; caso contrário, DEVE exigir revisão humana. Contato substituto citado na ausência NÃO DEVE virar destinatário sem aprovação.
- **R20.8:** Supressão e bloqueio prevalecem sobre qualquer retomada.
...
- **R21.7 [Proposta B2]:** Todo e-mail de sequência DEVE conter forma clara de saída; ONDE o provedor exigir descadastro técnico em um clique, o sistema DEVE oferecê-lo além do link visível, com processamento direto na supressão.
...
- **R26.3:** E-mail só DEVE passar a `habilitado` após T1 comprovado com endereços internos: envio, estados, recebimento, correlação de resposta e autenticação do domínio.
...
| AT13 | Mantido | — |
| AT14 | **Revisado** | Inclui as ações novas da tabela R9.2 (aprovar ficha: Gestor/Admin; consultar supressão: só Admin; Auditor só lê). |
...
| AT19 | R11.9 | DADO um consumidor final PME e um trader com a mesma aderência, ENTÃO o consumidor final vem antes e o trader continua visível e classificado. |
...
| 2026-09-22 | Traders ficam de fora? | Não. Consumidores finais PME têm prioridade; traders continuam elegíveis e classificados. | R2.4, R14 |
...
| 2026-09-22 | Metodologia dos textos? | `/prospeccao-vendas` v1.0 com adaptações A1–A11 e critérios PV1–PV8. | R17 |
...
Somente o que esta Spec **não** pode fechar sozinha. Nenhuma foi aprovada tacitamente.

| ID | Decisão | Proposta nesta Spec | Requisitos afetados | Bloqueia |
| --- | --- | --- | --- | --- |
| **B1** | Ausência automática e resposta incerta | Ausência identificada pausa só a pessoa; retomada automática só se prevista na ficha; incerta = resposta humana | R20.6, R20.7 | Implementação de R20.7. R20.6 já protege na dúvida. |
| **B2** | Saída clara e mecanismo técnico | Saída em todo e-mail + um clique onde o provedor exigir | R21.7 | Habilitação do e-mail (T1) |
| **B3** | Raio ajustável, limites, precisão, cobertura | Limites operacionais documentados; nova busca ao alterar raio; estimado/indeterminado | R11.11, R11.12, `param_radius_km_*` | Início de busca nacional real (Etapa 1) |
| **DS1** | Confidence e Potential no Nacional | Definir quais evidências de compra doméstica valem pontos e se as tabelas de açúcar/café valem no Nacional | R4.2, R5.1.2 | Qualificação Nacional (não bloqueia busca, ficha nem envio) |
| **DS2** | Moeda-base do Nacional | BRL no Nacional, USD no Internacional | R3.1.4 | Formulário de demanda Nacional |
| **DS3** | Nomes dos estados da trilha de prospecção e aposentadoria de `Prospectado` | Tabela §1.2 | §1.2, R1.3 | Fase 5 (design) |

Pendências comerciais que **não** são decisões da Spec: commodity, cidade/UF e raio do piloto (bloqueia Etapa 1); identidade de CSO (bloqueia só o item); valores dos parâmetros pendentes (R7.1); validação jurídica da retenção (T11).
...
| `/prospeccao-vendas` v1.0 (SHA-256 `94544246…acd0`) | Skill atualizada | R17.5, fichas afetadas |
...
| --- | --- |
| 1. Nacional: commodity + cidade/UF do fornecedor + raio ajustável → compradores | R11.1–R11.13, AT15–AT19 |
| 2. Internacional: país → histórico agrícola Brasil → commodities → empresas | R12.1–R12.10, R1.4, AT20–AT23 |
| 3. Consumidores finais prioritários; traders elegíveis | R2.4, R14, R11.9, AT12, AT19 |
| 4. Prospecção por commodity, sem lote, preço ou oferta | R1.1.2, R11.1, R12.1, R16.1–R16.2, R7.2.2, AT15 |
| 5. Textos finais aprovados por empresa; execução e interrupções | R17, R18, R19, R20, R21, R22, AT24–AT33 |

## 9. Controle do documento

- **Versão:** 2.0 — rascunho 1
- **Data:** 2026-09-22
- **Status:** aguardando aprovação de Rogério
