...
**Versão:** 2.0 — rascunho 2 da Fase 3
**Data:** 2026-09-22
**Status:** em revisão; **não aprovada integralmente**. Decisões parciais de Rogério em 2026-09-22 incorporadas (perfil Administrador, B2, parte de B3, DS2, DS3). Fase 3 não encerrada.
**Rascunho anterior:** `docs/historico/v2.0-planejamento/eag-compass-spec-v2.0-rascunho1.md`.
**Versão substituída:** Especificação v1.3 (2026-09-21), arquivada sem alteração em `docs/historico/v1.3/eag-compass-spec-v1.3.md`.
**Constituição associada:** `docs/eag-compass-constituicao.md` (v2.0, rascunho 2).
...
Tabelas da v1.3 mantidas como regra do mercado Internacional. No Nacional, até a decisão DS1, açúcar segue R4.3: tabela relativa a `M` se configurado; senão Volume e Anual ficam desconhecidos.
...
- **R5.1.2:** A dimensão "Evidência de compra" no mercado Nacional segue DS1, com proposta em §6.1. Até a decisão, o sistema DEVE aplicar a tabela v1.3 sem conceder pontos por registro alfandegário/BL a compras domésticas e DEVE exibir a nota "regra nacional de Confidence pendente".
...
| `param_radius_km_min`, `param_radius_km_max` | **sem valor** — a propor na Fase 4 com justificativa técnica; não copiar limites de fornecedores | Pendente (B3, parte restante) |
...
- **R11.14 [Pendente B3 — limites]:** SE o raio estiver fora de `param_radius_km_min`/`max`, ENTÃO o sistema DEVE recusar a busca e exibir o intervalo permitido. Os valores serão propostos na Fase 4, com justificativa técnica (fontes, geocodificação, custo e tempo por busca). Não serão copiados os limites da Econodata (20–200 km) nem do Google (≤ 50 km). Enquanto não houver valores aprovados, R7.1.1 impede a busca nacional **real**; testes internos com dublês não dependem deles.
...
| 2026-09-22 | DS1 — pontuação nacional? | Apresentar proposta com fórmulas e exemplos verificáveis. | §6.1 (proposta, não vigente) |
...
| **B3 — limites** | `param_radius_km_min`/`max` | Sem valores; busca nacional real impedida (R11.14, R7.1.1) | Etapa 1. Propor na Fase 4 com justificativa técnica. |
| **DS1** | Pontuação nacional (§6.1) | Tabela v1.3 sem pontos de alfândega/BL para compra doméstica, com aviso de regra pendente (R5.1.2); tabela de açúcar não usada no Nacional | Só a qualificação Nacional |
| DS1-b | Exigir `M_N` configurado para qualificar no Nacional? | Não exigido (mecânica de intervalo) | Idem |
| DS1-c | `M_N` é escala e mínimo ao mesmo tempo (como `M` na v1.3)? | Sim, como na v1.3 | Idem |

Pendências comerciais que **não** são decisões da Spec: commodity, cidade/UF e raio do piloto (bloqueia Etapa 1); identidade de CSO (bloqueia só o item); valores `M_N` por commodity e demais parâmetros pendentes (R7.1); validação jurídica da retenção (T11).

### 6.1 Proposta DS1 — pontuação do mercado Nacional (**não vigente**)
...
| EX-PN3 | Sem `M_N` configurado; demais dados como EX-PN1 | conhecidas 20 + 15 + 10 + 5 = 50; desconhecidas 30 + 20 = 50 | **PN = 50–100**; critério 3 atendido (ver DS1-b) |
| EX-PN4 | 60 t/op (0,6M); 12 op/ano, anual 720 t (7,2M); demais como EX-PN1 | 0 + 20 + 20 + 15 + 10 + 5 | **PN = 70** e `Abaixo do Mínimo` (R3.3.1, ver DS1-c) |
| EX-PN5 | Como EX-PN1, mas unidade por endereço a 180 km, raio 150 km | 20 + 20 + 20 + 15 + 10 + 0 | **PN = 85** |

Se DS1 for aprovada, os exemplos viram cenários AT40 em diante e R5.1.2 passa a apontar para esta tabela. A implementação exige alterar o motor 0.3.1 em três pontos: tabela relativa para qualquer commodity com `M` (hoje só café), catálogo de evidência por mercado e Atualidade condicionada a E (hoje é entrada independente).
...
| --- | --- |
| 1. Nacional: commodity + cidade/UF do fornecedor + raio ajustável → compradores | R11.1–R11.15, AT15–AT19, AT37, AT38 |
...
| 5. Textos finais aprovados por empresa; execução e interrupções | R17, R18, R19, R20, R21, R22, AT24–AT33, AT35, AT36 |

## 9. Controle do documento

- **Versão:** 2.0 — rascunho 2
- **Data:** 2026-09-22
- **Status:** decisões parciais incorporadas; aguardando aprovação integral de Rogério. Fase 3 aberta.
