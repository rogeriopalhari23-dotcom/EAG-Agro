...
export function reviewSequence(messages, ctx) {
  ...
  for (const r of decisors) {
    ...
    const ok = !!e1 && /^Olá/.test(e1.body) && /apenas iniciar uma conversa/.test(e1.body) && /20 minutos/.test(e1.body);
  ...
  for (const r of decisors) {
    const e1 = (byContact.get(r.contactId) || []).find((m) => m.step === 1);
    const says = e1 && /pelo LinkedIn/.test(e1.body);
    ...
    for (const m of messages.filter((x) => x.contactId === r.contactId && /em contato com .* por e-mail/i.test(x.body)))
  ...
  for (const r of decisors) {
    const list = byContact.get(r.contactId) || [];
    const breakIdx = list.findIndex((m) => /agora não é o melhor momento/.test(m.body));
  ...
  let pv7 = true;
  for (const m of messages) {
    if (m.channel !== "call" && FORBIDDEN_PT.test(m.body)) {
      pv7 = false;
    ...
    if (VOLUME_PT.test(m.body) && ctx.declarations?.volumeAvailable !== true) {
      pv7 = false;
  ...
  for (const m of emails.filter((x) => x.step === 1)) {
    const ok = /^Fornecedor [a-zà-ú][a-zà-ú ]*$/.test(m.subject || "") && !/[%\d]/.test(m.subject || "");
    finding(f, "PV9", ok, ok ? `Assunto "${m.subject}".` : `Assunto "${m.subject}" fora do padrão "Fornecedor [commodity]".`, { contactId: m.contactId });
  }
  // PV10: tom sem pedido de desculpas nem pedido de "setor de compras".
  const apologies = messages.filter((m) => APOLOGY_PT.test(m.body));
  ...
  for (const r of influencers) {
    ...
    const ok = !!mine && /20 minutos/.test(mine.body) && dec.every((d) => d.body !== mine.body);
  ...
  finding(f, "PV12", ctx.market !== "international" || !!ctx.languageGapNote, ctx.market !== "international" ? "Não se aplica (nacional)." : "Idioma e lacuna registrados.");
  // R19.13 / R21.7: endereço físico e saída em todo e-mail.
  for (const m of emails) {
    const ok = !!ctx.postalAddress && m.body.includes(ctx.postalAddress) && m.body.includes(ctx.unsubUrl(m.contactId)) && /"sair"/.test(m.body);
